...
class OutPark extends React.Component {
    ...
    checkBatch = (rule, value, callback) => {
        if ( value.length === 0 ) {
            return callback('请输入出园批次');
        }
        if (!/^\d+$/.test(value)) {
            return callback('请输入正确的出园批次');
        }
        callback();
    };
    ...
    scanProduct = () => {
        ...
        /*axios({
            ...
            if (stores.length === 0) {
                message.error('该出园批次需要出园的产品已全部出园！');
                return;
            }
            ...
            <Card title={'执行出园'}>
                { !begin ? (
                    ...
                    <Form layout={"inline"}>
                        ...
                        <Form.Item>
                            { allCompleted ? (
                                <Alert message="本次出园已完成" type="success" />
                            ) : (
...
export default withRouter(Form.create()(OutPark));
